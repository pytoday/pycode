...
import sys, os, time, subprocess, argparse, configparser
import wget, tarfile
...
def install(conf_name, password, port):
    ...
    # change password
    ch_cmd = "/usr/local/mysql/bin/mysql -S /tmp/mysql"+port+".sock"+" -uroot -e \"alter user user() identified by '"\
             + password+"';\""
    i = 0
    while i < 5:
        i += 1
        ...
        if not p[0]:
            chpasswd = subprocess.getstatusoutput(ch_cmd)
            if not chpasswd[0]:
                ins_log.info("Change password SUCCESS. Stop mysqld for now...")
                subprocess.getstatusoutput("killall mysqld")
                break
        else:
            ins_log.warning("Change password failed. May be you can change it manual(%d)." % i)
...
if __name__ == '__main__':
    ...
    install(conf_file, password, port)
